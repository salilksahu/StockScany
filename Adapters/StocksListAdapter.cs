...
{
    class StocksListAdapter : BaseAdapter
    {
        ...
        int db_state = 0;
        string account_type;
        ImageButton btnShowChart;
        ImageButton btnAddWatch;
        public StocksListAdapter(Context context, List<StockItem> items)
        {
            this.items = items;
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            StocksListAdapterViewHolder holder = null;

            if (view != null)
                holder = view.Tag as StocksListAdapterViewHolder;

            if (holder == null)
            {
                holder = new StocksListAdapterViewHolder();
                var inflater = context.GetSystemService(Context.LayoutInflaterService).JavaCast<LayoutInflater>();
                //replace with your item and your holder items
                //comment back in
                view = inflater.Inflate(Resource.Layout.stock_item, parent, false);
                //holder.Title = view.FindViewById<TextView>(Resource.Id.text);                
                
                btnShowChart = view.FindViewById<ImageButton>(Resource.Id.btnShowChart);                
                btnAddWatch = view.FindViewById<ImageButton>(Resource.Id.btnAddWatch);
                
                btnShowChart.Click += (sender, args) =>
                {
                    
                    int iTag = Int32.Parse(((ImageButton)sender).Tag.ToString());
                    string sSymb = "";
                    ...
                    LoadStockChart(iTag, sSymb);
                };                

                btnAddWatch.Click += (sender, args) =>
                {
                    if (db_state == 0)
                        GetAccountDetails();
            ...
            if (item.stock_price_per < 0)
                view.FindViewById<TextView>(Resource.Id.txtStockPricePer).SetTextColor(Android.Content.Res.ColorStateList.ValueOf(Android.Graphics.Color.Red));
            else
            ...
            string sPer = Decimal.Round(Decimal.Parse((item.trade_value / 10000000).ToString()), 2).ToString();
            view.FindViewById<TextView>(Resource.Id.txtTrdValue).Text = "Trade Value " + sPer + " Cr";
            view.FindViewById<TextView>(Resource.Id.textStockOpen).Text = "Open " + item.open_p.ToString();
            view.FindViewById<TextView>(Resource.Id.textStockHigh).Text = "High " + item.high_p.ToString();
            view.FindViewById<TextView>(Resource.Id.textStockLow).Text = "Low " + item.low_p.ToString();
            view.FindViewById<TextView>(Resource.Id.textStockClose).Text = "Close " + item.close_p.ToString();
            view.FindViewById<ImageButton>(Resource.Id.btnShowChart).Tag = item.sec_id;
            view.FindViewById<ImageButton>(Resource.Id.btnAddWatch).Tag = item.sec_id;
        ...
        public void LoadStockChart(int sec_id, string sSymbol)
        {
            try
            {
                ...
                candleStickChart.LayoutParameters.Height = (int)mainDisplayInfo.Height - (int)(3 * mainDisplayInfo.Height / 4);
            ...
            {
                DisplayAlert("Loading Chart", "There was some issue loading the stock chart. Please try after some time.");
                return;
            }
        }

        public void AddToWatchlist(int sec_id, string sSymbol)
        { 
            string _databasePath = Xamarin.Essentials.FileSystem.AppDataDirectory + "/SqliteDatabase.db3";
            var db = new SQLiteConnection(_databasePath);
            ...
            if (table.Count > 0)
            {
                DisplayAlert("Adding to Watchlist", sSymbol + " is already in your watchlist");
                return;
            }
            else
            {
                var table1 = db.Query<WLIDS>("SELECT * FROM [WLIDS] WHERE sec_id = ? ", sec_id);
                if (table1.Count >= 30)
                {
                ...
                else
                {
                    ...
                    db.Insert(wlid);
                    DisplayAlert("Adding to Watchlist", sSymbol + " has been added to your watchlist.");
                } 
            }
            db.Close();
    ...
    class StocksListAdapterViewHolder : Java.Lang.Object
    {
